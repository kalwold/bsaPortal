...
export const extractIfbLoanRangeRegionQuarterlyMetadata = (data) => {
  const metadata = {
    reportTitle: "",
    ReturnKey: "",
    institutionCode: "",
...
if (firstCell.includes('INT_LON_R&R_EE002') ) {
        metadata.reportType = 'ifb-quarterly_loan-range';
        metadata.reportTypeId = 'ifb-quarterly_loan-range';
    ...
    if (
      ( i === 12) &&
      (thirdCell || sixteenCell || firstCell) &&
...
const extractIfbLoanRangeRegionQuarterlyData = (data) => {
    ...
    for (let j = 0; j < loanRanges.length; j++) {
     ...
     const accountIndex = 4 + (j * 3);
      ...
      if (borrowersIndex < row.length) {
        const val = parseFloat(String(row[borrowersIndex] ?? "").replace(/[,%\s]/g, ""));
        if (!isNaN(val) && val !== 0) {
...
export default extractIfbLoanRangeRegionQuarterlyData