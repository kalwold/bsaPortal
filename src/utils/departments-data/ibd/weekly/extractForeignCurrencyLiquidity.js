...
/* ------------------------------------------------------------------ *
 *  Weekly Foreign Currency Liquidity  (A1 = "WFWFCL001")
 *
 *  Layout:
 *    A No. | B FX (In USD, In Euro, In Pound Sterling, In Chinese Yuan,
 *                   In Canadian Dollar, In Japanese Yen, then Total)
 *    C..D  Foreign Currency Current Liabilities (maturing up to less than one year)
 *            -> External Borrowings, FCY Liabilities Other than External Loans
 *    E Total FCY Current Liabilities | F 15% of the FCY Current Liabilities |
 *    G Foreign Currency Current Assets # | H Excess (shortage) in liquidity
 *
 *  Header = 3 rows (rows 9-11). Row 12 is the formula legend (A, B, C=A+B,
 *  D=15%*C, E, F=E-D): it has no label in A/B so the data loop skips it.
 *
 *  IDENTIFICATION RULE: a value column under a parent header gets the parent
 *  name prepended ("<Parent>_<Child>"); columns with no parent keep their name.
 *  All value columns are numeric. E, F, H and the Total row are formulas.
 * ------------------------------------------------------------------ */
