...
export const extractDepositRangeRegionQuarterlyMetadata = (data) => {
  const metadata = {
    reportTitle: "",
    ReturnKey: "",
    institutionCode: "",
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const therteenCell = String(row[11] || "").trim();
    ...
    if (firstCell.includes('DEP_RAN&REG_DR002')) {
        metadata.reportType = 'finance-quarterly_deposit-range-region';
        metadata.reportTypeId = 'finance-quarterly_deposit-range-region';
    ...
    if (
      ( i === 12) &&
      (thirdCell || therteenCell || firstCell) &&
      (thirdCell.toLowerCase().includes("in") ||
        therteenCell.toLowerCase().includes("in") || firstCell.toLowerCase().includes('In'))
    ) {
      metadata.unit =  therteenCell || '';
...
const extractDepositRangeRegionQuarterlyData = (data) => {
...
export default extractDepositRangeRegionQuarterlyData